<script lang="ts">
  import type { Course, Experience, ExperienceFreelance } from "../../dataType";
  import Item from "../Item.svelte";

  export let course: Course;

  function getYearDate(time: number) {
    return new Date(course.startDate).getFullYear();
  }

  const startYear = getYearDate(course.startDate);
  const endYear = getYearDate(course.endDate);
  ...
  const endDateString = course.endDate
